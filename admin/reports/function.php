...
{        
	global $connection;
	$sql = "SELECT STN_ID,STN_CODE,STN_NAME_THAI FROM [PATTANI].[dbo].[TM_STN] order by LOC_ID";
	...
	while($row = odbc_fetch_array($result))
	{
		...
		echo "<option value=\"".$row['STN_ID']."-".$nane."\" ".$sl.">".$row['STN_CODE']."-".$nane."</option>";
	}
}
function getSiteList1($STN_Code)
{        
	global $connection;
	$sql = "SELECT STN_ID,STN_CODE,STN_NAME_THAI FROM [PATTANI].[dbo].[TM_STN] order by STN_ID";
	$result = odbc_exec($connection,$sql);
	$sta = explode("-", $STN_Code);
	$ssite = $sta[0];
	while($row = odbc_fetch_array($result))
	{
		$nane = iconv('TIS-620', 'UTF-8', $row['STN_NAME_THAI']);
		if($ssite==$row['STN_ID']) { $sl = "selected"; } else { $sl = ""; }
		echo "<option value=\"".$row['STN_ID']."\" ".$sl.">".$row['STN_ID']."-".$nane."</option>";
	}
}
